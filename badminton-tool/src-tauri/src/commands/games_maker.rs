...
fn create_balanced_teams(players: Vec<Player>, num_courts: usize) -> Vec<Vec<Player>> {
...
fn optimize_teams_stability(teams: &mut Vec<Vec<Player>>) {
...
fn find_best_swap(team_a: &[Player], team_b: &[Player]) -> Option<(usize, usize)> {
    ...
    for (i, player_a) in team_a.iter().enumerate() {
        for (j, player_b) in team_b.iter().enumerate() {
            ...
            let new_team_a_total = team_a.iter().map(|p| p.skill_level).sum::<i32>() 
                - player_a.skill_level + player_b.skill_level;
            let new_team_b_total = team_b.iter().map(|p| p.skill_level).sum::<i32>() 
                - player_b.skill_level + player_a.skill_level;
...
fn calculate_team_average(team: &[Player]) -> f64 {
    if team.is_empty() {
        return 0.0;
    }
